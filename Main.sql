...
BEGIN
    ...
    INSERT INTO Address (AddressID, CustomerID, Street, City, State, ZipCode, AddressType)
    VALUES (Address_seq.NEXTVAL, v_customer_id,  p_shipping_address, p_shipping_city, p_shipping_state, p_shipping_zip, p_address_type_s)
    RETURNING AddressID INTO v_shipping_address_id;

    INSERT INTO Address (AddressID, CustomerID, Street, City, State, ZipCode, AddressType)
    VALUES (Address_seq.NEXTVAL, v_customer_id,  p_billing_address, p_billing_city, p_billing_state, p_billing_zip, p_address_type_b)
    RETURNING AddressID INTO v_billing_address_id;

    UPDATE Customer
    SET ShippingAddressID = v_shipping_address_id,
        BillingAddressID  = v_billing_address_id
...
BEGIN
    ...
    INSERT INTO Admin (FirstName, LastName, UserID, Email,Phone, Address, City, State, ZipCode)
    VALUES (p_first_name, p_last_name, v_user_id, p_email, p_phone,p_address, p_city, p_state, p_zip_code);
...
BEGIN
    ...
    INSERT INTO Driver (FirstName, LastName, UserID, LicenseNumber, Email, Phone, Address, City, State, ZipCode)
    VALUES (p_first_name, p_last_name, v_user_id, p_license_number, p_email, p_phone, p_address, p_city, p_state, p_zip_code);
...
BEGIN
    ...
    INSERT INTO Manager (FirstName, LastName, UserID, Email, Phone, Address, City, State, ZipCode) 
    VALUES (p_first_name, p_last_name, v_user_id, p_email, p_phone, p_address, p_city, p_state, p_zip_code);
...
BEGIN
    add_customer(
        ...
        p_shipping_state => 'KL',
        p_shipping_zip => '1200',
        p_address_type_s => 'ShippingAddress',
        p_billing_address => '396-A School Road',
        p_billing_city => 'Panadura',
        p_billing_state => 'KL',
        p_billing_zip => '1200',
        p_address_type_b => 'BillingAddress'
    );
END;
/
--    AddressType VARCHAR2(50) CHECK (AddressType IN ('ShippingAddress', 'BillingAddress')),
...
BEGIN
    add_admin('Hansana', 'Adhikari', 'hansana@gmail.com', 'hansana2001', 'password123', '123456789V', '72829272','456 Supply Lane','Los Angeles','CA', '9001');
END;
/
...
BEGIN
    add_driver(
        ...
        p_phone           => '4353638',  -- Removed the trailing comma
        p_address         => '456 Supply Lane',
        p_city            => 'Los Angeles',
        p_state           => 'CA',
        p_zip_code        => '90001'
...
BEGIN
    add_manager('mathesha', 'Silva', 'matheesha@gmial.com', 'matheesha1234','password123', '456789123V','0718927372','456 Supply Lane','Los Angeles','CA', '9001');
END;
/
...
BEGIN
    INSERT INTO Supplier (
        ...
        p_SupplierName, p_ContactName, p_Email, p_Phone,
...
BEGIN
    OPEN o_Supplier FOR
        SELECT SupplierID, SupplierName, ContactName, Email, Phone, Address, City, State, ZipCode
        FROM Supplier
        WHERE SupplierID = p_SupplierID;
END;
...
BEGIN
    ...
    INTO v_SupplierID 
    FROM Supplier 
    WHERE SupplierName = 'ABC Supplies';
...
BEGIN
    ...
    INTO v_CustomerID 
    FROM Customer 
    WHERE Email = 'manurasanjula2003@example.com';
    ...
    DBMS_OUTPUT.PUT_LINE('Inserted Order');
EXCEPTION
    WHEN NO_DATA_FOUND THEN
...
DECLARE
    v_SupplierID Supplier.SupplierID%TYPE;
BEGIN
    -- Retrieve the SupplierID for 'ABC Supplies'
    SELECT SupplierID 
    INTO v_SupplierID 
    FROM Supplier 
    WHERE SupplierName = 'ABC Supplies';
    ...
    Update_Supplier(
        p_SupplierID    => v_SupplierID,
        p_SupplierName  => 'ABC Supplies',
        p_ContactName   => 'John Doe',
        p_Email         => 'john.doe@abcsupplies.com', -- Corrected parameter name
        p_Phone         => '123-456-7890',
        p_Address       => '123 Tech Street',
        p_City          => 'Techville',
        p_State         => 'TechState',
    ...
    DBMS_OUTPUT.PUT_LINE('Updated Supplier Contact Email for ABC Supplies.');
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('Supplier not found.');
    WHEN TOO_MANY_ROWS THEN
        DBMS_OUTPUT.PUT_LINE('Multiple suppliers found with the name ABC Supplies.');
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('An unexpected error occurred: ' || SQLERRM);
END;
/
...
BEGIN
    ...
    SELECT CustomerID, ShippingAddressID, BillingAddressID 
    INTO v_CustomerID, v_ShippingAddressID, v_BillingAddressID  
    FROM Customer 
    WHERE Email = 'manurasanjula2003@example.com';
...
--    INTO v_ShippingAddressID 
--    FROM Address 
--    WHERE CustomerID = v_CustomerID AND AddressType = 'ShippingAddress';
--
--    -- Retrieve Billing AddressID ('Work')
--    SELECT AddressID 
--    INTO v_BillingAddressID 
--    FROM Address 
--    WHERE CustomerID = v_CustomerID AND AddressType = 'BillingAddress';
...
DECLARE
    ...
    v_Email Supplier.Email%TYPE; -- Renamed from ContactEmail to Email
...
BEGIN
    ...
    LOOP
        ...
        DBMS_OUTPUT.PUT_LINE('Contact Email: ' || v_Email); -- Updated variable
